<?php
    ...
    if(isset($_POST['get_rooms_images'])){

        $frm_data = filteration($_POST);
        $res = select("SELECT * FROM `room_images` WHERE `room_id`=?",[$frm_data['get_rooms_images']],'i');
    ...
    if(isset($_POST['remove_room'])){
        ...
        $res5 = update("UPDATE FROM `rooms` SET `removed`=?  WHERE `id`=?",[1,$frm_data['room_id']],'ii');
